...
MODEL_PATH = os.path.join(THIS_DIR, "phish_model_rf_combined_cv.joblib")
...
def main():
    ...
    print(f"[INFO] RF best params: {search.best_params_}")
    print(f"[INFO] RF best CV F1: {search.best_score_:.4f}")

    best_model = search.best_estimator_
    y_pred = best_model.predict(X_test)

    print_results("RANDOM FOREST", y_test, y_pred)
