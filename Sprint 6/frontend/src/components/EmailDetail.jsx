...
function EmailDetail({ email }) {
  ...
  const bodyFindingCount = findings.filter((f) => f.field === "body").length;
  const subjectFindingCount = findings.filter((f) => f.field === "subject").length;
  const senderFindingCount = findings.filter((f) => f.field === "sender").length;

  const hasVisibleHighlights =
    bodyFindingCount > 0 || subjectFindingCount > 0 || senderFindingCount > 0;
  ...
  return (
    <div className="email-detail">
      <header className="email-detail-header">
        ...
        {email.aiExplanation && (
          <div className="email-ai-explanation">{email.aiExplanation}</div>
        )}

        {shouldHighlight && !hasVisibleHighlights && (
          <div className="email-ai-explanation">
            This email was flagged by the model, but no specific suspicious text
            spans were extracted.
          </div>
        )}
      </header>
