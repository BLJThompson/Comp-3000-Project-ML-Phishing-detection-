...
function addSenderEmailFinding(
  findings,
  sender,
  emailAddress,
  type,
...
function extractFindings(email) {
  ...
  // Soft urgency rule first as low severity
  addMatches(
    findings,
    body,
    "body",
  ...
  // URL rules — much stricter than before
  const urlRegex = /https?:\/\/[^\s)>\]]+/gi;
  const shortenerDomains = ["bit.ly", "tinyurl", "goo.gl", "t.co", "ow.ly", "buff.ly"];
  const safeUrlHints = [
    "mailman/listinfo",
    "unsubscribe",
    "privacy",
  ...
  for (const match of body.matchAll(urlRegex)) {
    ...
    const isSafeHint = safeUrlHints.some((hint) => lower.includes(hint));
    const isShortener = shortenerDomains.some((domain) => lower.includes(domain));
    const hasSuspiciousKeyword = suspiciousUrlHints.some((hint) =>
      lower.includes(hint)
    );
  ...
  if (emailAddress) {
    for (const brand of knownBrands) {
      ...
      const subjectContainsBrand = lowerSubject.includes(brand.name);
      const bodyContainsBrand = lowerBody.includes(brand.name);
  ...
  // Upgrade urgency if paired with stronger phishing intent
  ...
  if (shouldUpgradeUrgency) {
    for (const f of upgradedFindings) {
      if (f.type === "urgency") {
        f.severity = "medium";
        f.reason =
          "Uses urgency language alongside stronger phishing indicators.";
      }
    }
  }
